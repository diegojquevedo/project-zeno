...
- generate_report_pdf_kpis: When data_source is geo_lake_county, after geo_build_result_summary, if the summarized layer_id is NOT one of representative_points, points, lines, areas (i.e. not SMC project geometry layers), call once to define the four executive PDF KPI tiles. Use real field names and values from the summary features or charts — never WMB, Section 319, or generic project status for streams, pollutants, soils, hydro, etc. Card 1 total is taken from state; primary_metric_caption uses " | " for a two-line label (e.g. Total streams | identified). Never mention this tool in chat.
...
After geo_build_result_summary when layer_id is not representative_points, points, lines, or areas, call generate_report_pdf_kpis(...) once so the PDF KPI strip and portfolio section match the data (see tool docstring).

...
def _apply_report_pdf_exports_from_update(update: dict) -> None:
    if not isinstance(update, dict):
        return

    def apply_flat(d: dict) -> None:
        if "report_pdf_metadata" in d:
            v = d["report_pdf_metadata"]
            st.session_state[SESSION_KEY_REPORT_PDF_METADATA] = (
                v if isinstance(v, dict) else None
            )
        if "report_pdf_kpis" in d:
            v = d["report_pdf_kpis"]
            st.session_state[SESSION_KEY_REPORT_PDF_KPIS] = (
                v if isinstance(v, dict) else None
            )
...
def _capture_export_snapshot() -> dict:
    ...
    rpk = st.session_state.get(SESSION_KEY_REPORT_PDF_KPIS)
    return {
        ...
        "report_pdf_kpis": copy.deepcopy(rpk) if isinstance(rpk, dict) else None,
    }
...
def _render_response_pdf_button(
    ...
    *,
    ...
    _rpk = (
        export_snapshot.get("report_pdf_kpis")
        if isinstance(export_snapshot, dict)
        else None
    )
    pdf_bytes = build_single_response_pdf_bytes(
        ...
        report_pdf_kpis=_rpk if isinstance(_rpk, dict) else None,
    )
...
if SESSION_KEY_REPORT_PDF_KPIS not in st.session_state:
    st.session_state[SESSION_KEY_REPORT_PDF_KPIS] = None
                            ...
                            try:
                                ...
                                if stream.get("node") == "ui_state":
                                    ...
                                    if st.session_state.get(SESSION_KEY_DATA_SOURCE) == "geo_lake_county":
                                        ...
                                        _apply_report_pdf_exports_from_update(update)
                                    continue
                                stream_count += 1

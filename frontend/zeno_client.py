...
class ZenoClient:
    ...
    def delete_thread(self, thread_id: str):
        ...
        url = f"{self.base_url}/api/v1/threads/{thread_id}"
    ...
    def fetch_lake_county_boundary(self) -> Dict[str, Any]:
        ...
        url = f"{self.base_url}/api/v1/lake_county/boundary"
        headers = {}
        if self.token:
            headers["Authorization"] = f"Bearer {self.token}"
        with requests.get(url, headers=headers) as response:
    ...
    def fetch_lake_county_layers(self) -> List[Dict[str, Any]]:
        ...
        url = f"{self.base_url}/api/v1/lake_county/layers"
        headers = {}
        if self.token:
            headers["Authorization"] = f"Bearer {self.token}"
        with requests.get(url, headers=headers) as response:
    ...
    def fetch_lake_county_features(
        ...
        """
        url = f"{self.base_url}/api/v1/lake_county/{layer_id}/features"
    ...
    def fetch_geometry(self, source: str, src_id: str):
        ...
        """
        url = f"{self.base_url}/api/v1/geometry/{source}/{src_id}"
        headers = {}
        if self.token:
            headers["Authorization"] = f"Bearer {self.token}"
    ...
    def get_thread_state(self, thread_id: str):
        ...
        """
        url = f"{self.base_url}/api/v1/threads/{thread_id}/state"
        headers = {}
        if self.token:
            headers["Authorization"] = f"Bearer {self.token}"
    ...
    def download_data(
        ...
        url = (
            f"{self.base_url}/api/v1/threads/{thread_id}/{checkpoint_id}/raw_data"
        )
        print("URL: ", url)
        headers = {
    ...
    def get_quota_info(self):
        url = f"{self.base_url}/api/v1/quota"
        headers = (
...
def main():
    ...
    Usage:
        python zeno_client.py [query] [--persona PERSONA] [--thread-id THREAD_ID]
    """
